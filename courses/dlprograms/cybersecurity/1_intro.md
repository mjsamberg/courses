...
A specific type of phishing, called an **Account Takeover Attack** is used when a legitimate account is compromised, and then used to conduct the phishing attack. These types of attacks typically have greater success as they appear to be from legitimate sources, even if the requests are odd.
